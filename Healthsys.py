...
def lipanampesa():
    global payments,AMOUNT,PHONE,currentorder,lbl_error,amountpayable
    payments = Tk()
    ...
    PHONE = IntVar()
    AMOUNT= IntVar()
    TopLoginForm = Frame(payments, width=400, height=50, bd=1, relief=SOLID)
    TopLoginForm.pack(side=TOP, pady=20)
    lbl_text = Label(TopLoginForm, text="Recieve Payments Via Mpesa", font=('arial', 12), width=400)
    lbl_text.pack(fill=X)
    MidLoginForm = Frame(payments, width=250)
    MidLoginForm.pack(side=TOP, pady=40)
    lbl_password = Label(MidLoginForm, text="PHONE NUMBER:", font=('arial', 20), bd=15)
    lbl_password.grid(row=1)
    lbl_error = Label(MidLoginForm, text="ENTER PHONE NUMBER AS: 2547XXXXXXXX", font=('arial', 15), bd=15, fg="red")
    lbl_error.grid(row=4, columnspan=2,pady=15)
    
    PHONE = Entry(MidLoginForm, textvariable=PHONE, font=('arial', 25), width=15)
    PHONE.grid(row=1, column=1)
    #start
    lbl1_password = Label(MidLoginForm, text="Amountpaid:", font=('arial', 20), bd=15)
    lbl1_password.grid(row=2)
    AMOUNT = Entry(MidLoginForm, textvariable=PHONE, font=('arial', 25), width=15)
    AMOUNT.grid(row=2, column=1)
    #end
    
    btn_login = Button(MidLoginForm, text="Recieve", font=('arial', 18), width=30, command=mpay)
    btn_login.grid(row=3, columnspan=3, pady=20)
    currentorder.focus
    btn1_login = Button(MidLoginForm, text="Printreceipt", font=('arial', 18), width=30, command=reciept_pdf)
    btn1_login.grid(row=6, columnspan=4, pady=20)
    #currentorder.focus
  
#========================================VARIABLES#=============================================================================================================================================== 

root = Tk()
root.title("Pharmacy Health")
...
def Database():
    global conn, cursor
    conn = sqlite3.connect("healthsystem.db")
    cursor = conn.cursor()
...
#========================================exit#===============================================================================================================================================
def Exit():
    result = tkMessageBox.askquestion('@healthsystem', 'Are you sure you want to exit?', icon="warning")
    if result == 'yes':
        LoginForm()
...
def LoginForm():
    ...
    TopLoginForm.pack(side=TOP, pady=20)
    lbl_text = Label(TopLoginForm, text="User Login Panel @ Pharmacy Sys", font=('arial', 18), width=600)
    lbl_text.pack(fill=X)
...
def adduser_form():
    ...
    adminform = Tk()
    adminform.title("DERRICK DEVELOPER LOGIN PANEL")
    width = 900
    height = 520
...
def recieve_payments():
    ...
    payments = Tk()
    payments.title("DERRICK THE DEVELOPER")
    width = 700
    height = 520
    screen_width = payments .winfo_screenwidth()
    screen_height = payments .winfo_screenheight()
    ...
    TopLoginForm.pack(side=TOP, pady=20)
    lbl_text = Label(TopLoginForm, text="Recieve cash", font=('arial', 18), width=600)
    lbl_text.pack(fill=X)
...
def update_stock():
    ...
    NEWSTOCK = Tk()
    NEWSTOCK.title("DERRICK THE DEVELOPER")
    width = 900
    height = 520
...
def Home():
    global Home
    Home = Tk()
    Home.title("DERRICK THE DEVELOPER @admin")
    width = 1024
    height = 600
    ...
    lbl_display = Label(Title, text="LIST OF OUR DRUGS IN FACILITY", font=('arial', 45))
    ...
    filemenu6.add_command(label="mpesa", command=ShowView)
    ...
    menubar.add_cascade(label="mpesa", menu=filemenu6)
...
def instock():
    instock = Tk()
    instock.title("DERRICK THE DEVELOPER @stock")
    width = 400
    height = 400
...
def Delete_transaction():
    ...
    Delete_transaction.title("POINT OF SALE@delete")
    width = 600
    height = 300
...
def reciept_pdf():
    ...
    pdf.add_page()
    pdf.cell(0,1, 'DERRICK AMSDER', 0, ln=1)
    pdf.set_font('Arial','B',12)
    pdf.cell(0,1, 'CELL: 2540746215773', 0, ln=1)
...
def lipa():
    global amountpaid,bal,OrDerNum,total
...
def Cashier():
    ...
    not_executed = 1
    cashier = Tk()
    cashier.title("DERRICK THE DEVELOPER")
    ...
    lbl_TopForm = Label(TopForm, text="MERU UNIVERSITY PHARMACY SYSTEM", font=('Arial Black', 24),bg="brown", fg ="powder blue")
    ...
    btn_s = Button(LeftFormt, text="Empty receipt", width=18, font=('arial', 22), bg="#009ACD", command=removefrom_cart)
    btn_s.grid(row=7, column=0, pady=10)
    btn_s = Button(LeftFormt, text="Add to receipt", width=18, font=('arial', 22), bg="#009ACD", command=addtocart)
...
#RIGHT HAND FRAME
    ...
    lbl_MidForm = Label(MidForm, width=512, text="LIST OF DRUGS SALES", font=('arial', 24),bg="#009ACD")
    ...
    lbl_MidForm = Label(MidForm, width=512, text="Developed by: Derrick Contact: 254719842513", font=('arial', 20),bg="powder blue")
    lbl_MidForm.pack(side=TOP,fill="both")
    ...
    lipamenu.add_command(label="Recieve cash", command=recieve_payments)
    ...
    mpesamenu = Menu(menu, font = ('Verdana', 14),tearoff=0)
    menu.add_cascade(label="Receive mpesa", menu=mpesamenu)
    mpesamenu.add_command(label="Receive", command=lipanampesa)
    ...
    menu.add_cascade(label="Receipt Ticket", menu=reportmenu)
    reportmenu.add_command(label="Ticket", command=print)
...
def print():
    ...
    for child in listBox.get_children():
        ...
        item1 = Label(top, text=f'{item}')
        item1.config(bg="white")
        item1.pack()
 
    amount = Label(top, text=f'Total\t{amount}')
    amount.config(bg="white")
    amount.pack()
...
#========================================LABEL WIDGET=====================================
lbl_display = Label(Title, text="DERRICK THE DEVELOPER", font=('arial', 50))
lbl_display.pack()
ShowLoginForm()
root.withdraw()
#========================================INITIALIZATION===================================
